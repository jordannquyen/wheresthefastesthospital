...
function AuthGate() {
  ...
  if (user.role !== pageRole)
    return <RoleMismatchScreen current={user.role} needed={pageRole} />;
        ...
        <div className="grid flex-1 items-center gap-8 py-10 lg:grid-cols-[1.05fr_0.95fr] lg:py-12">
          <section>
            <p className="font-mono text-xs uppercase tracking-[0.24em] text-cyan-300">
              voice intake to hospital acceptance
            </p>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">
              Turn EMT voice reports into structured patient records, rank
              nearby hospitals by capacity and ETA, and give receiving teams a
              live incoming-patient queue.
            </p>
          ...
          <section className="overflow-hidden rounded-xl border border-slate-700 bg-slate-950/70 shadow-2xl shadow-black/40">
            ...
            <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-1">
              <img
                src={logo}
                alt="Vital-Route"
                className="mx-auto h-44 w-auto object-contain p-8 sm:h-full lg:h-52"
              />
              <div className="border-t border-slate-700 p-5 sm:border-l sm:border-t-0 lg:border-l-0 lg:border-t">
                <div className="space-y-3 text-sm">
                  {[
                    ...
                    <div
                      key={step}
                      className="flex items-center gap-3 rounded-md border border-slate-700 bg-slate-900/70 p-3"
                    >
                      <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-cyan-400/15 font-mono text-xs font-bold text-cyan-300">
                        {step}
                      </span>
...
function AuthenticatedApp({ user }) {
  ...
  const [activeTab, setActiveTab] = useState(
    isHospitalPage ? "hospital" : isAdminMode ? "admin" : "emt",
  );
  ...
  useEffect(() => {
    ...
    async function initLocation() {
      ...
      if (!loc) {
        ...
        source = "default";
        console.warn(
          "Using LA default location — allow browser location or type an address to override.",
        );
      }
  ...
  useEffect(() => {
    ...
    if (dispatch.status === "accepted" && prev.status !== "accepted") {
      ...
      voice.speak(
        "All hospitals have diverted. Please contact dispatch for further instructions.",
      );
      ...
      if (autoApproved) {
        voice.speak(
          `${prevHospital?.hospitalName} diverted. Auto-approved at ${hospital?.hospitalName}, ${eta} minutes away. En route.`,
        );
      } else {
        voice.speak(
          `${prevHospital?.hospitalName} diverted. Escalating to ${hospital?.hospitalName}, ${eta} minutes away. Awaiting confirmation.`,
        );
      }
    }
  }, [dispatch]);
      ...
      .then((data) => {
        setAdminHospitals(
          nodes.map((n) => ({
            hospitalId: n.id,
            hospitalName: n.name,
            override: data.overrides?.[n.id] ?? null,
          })),
        );
      })
      .catch(() => {
  ...
  async function fetchHospitalRequests() {
    if (user.role !== "hospital" || !user.hospitalId) return;
    const hospitalParam =
      user.hospitalId === "__all__" ? "__all__" : user.hospitalId;
    const res = await apiFetch(
      `/api/hospitals/${hospitalParam}/incoming-patients`,
    );
    const data = await res.json();
    const cards = [];
    for (const patient of data.patients ?? []) {
      ...
      if (patient.divertHistory?.length) {
        for (const entry of patient.divertHistory) {
          if (
            user.hospitalId === "__all__" ||
            entry.hospitalId === user.hospitalId
          ) {
  ...
  async function handlePatientHandoff() {
    ...
    const patientId =
      dispatch?.activeRequest?.patientId ??
      dispatch?.patientId ??
      currentPatientId ??
    ...
    if (patientId) {
      await apiFetch(`/api/patients/${patientId}/deliver`, {
        method: "PATCH",
      }).catch(() => {});
    }
  ...
  async function handleRequestAction(request, status) {
    if (request.source === "patient") {
      if (status === "accepted") {
        ...
        await apiFetch(`/api/patients/${request.patientId}/divert`, {
          method: "PATCH",
        });
      }
      fetchHospitalRequests();
  ...
  async function handleTalkToggle() {
    if (voice.isListening) {
      ...
      try {
        ...
        summary = extractRes.ok
          ? await extractRes.json()
          : extractPatient(transcript);
      } catch {
      ...
      const patientPayload = {
        ...
        address:
          geocoded?.formattedAddress ?? summary.address ?? resolvedAddress,
      };
      ...
      if (resolvedOrigin) {
        const routeData = await requestRecommendations(resolvedOrigin, {
          insurance: summary.insuranceProvider,
          patientId: summary.patientId,
          condition: summary.condition,
        });
        const dispatchData = await autoDispatch(
          routeData,
        ...
        voice.speak(
          `${reply} Dispatching to ${hospital?.hospitalName ?? "top hospital"}, ${eta} minutes away. ${statusPhrase}`,
        );
      } else {
        voice.speak(reply);
      }
  ...
  async function requestRecommendations(inputOrigin, overrides = {}) {
    ...
    const effectiveCondition =
      overrides.condition ?? patientSummary?.condition ?? null;
      ...
      <main className="screen bg-grid text-slate-100">
        <div className="mx-auto max-w-3xl rounded-md border border-cyan-400/40 bg-slate-900/90 p-8 shadow-2xl shadow-cyan-600/20">
          ...
          <p className="mt-3 text-slate-300">
            Add GOOGLE_MAPS_API_KEY to your .env and restart.
          </p>
        </div>
      </main>
    );
  ...
  if (loadError) {
    return (
      <main className="screen bg-grid p-8 text-red-300">
          ...
          <section className="grid min-h-0 grid-cols-1 gap-4 lg:grid-cols-[2fr_1fr]">
            <article className="relative min-h-[420px] overflow-hidden rounded-xl border border-slate-700 bg-slate-950/70 shadow-xl shadow-black/35">
              {isLoaded && (
                ...
                >
                  {(route?.top3?.length ? route.top3 : nodes).map((node) => {
                  ...
                  {origin && (
                    <MarkerF position={origin} title="Patient Origin" />
                  )}
                  ...
                  {origin && route?.closest && (
                    <Polyline
                      path={[
                        origin,
                        { lat: route.closest.lat, lng: route.closest.lng },
                      ]}
                      options={{
                        strokeColor: "#94a3b8",
                        strokeOpacity: 0,
                  ...
                  {origin && route?.recommended && (
                    <Polyline
                      path={[
                        origin,
                        {
                          lat: route.recommended.lat,
                          lng: route.recommended.lng,
                        },
                      ]}
                      options={{
                        strokeColor: "#38bdf8",
                    ...
                    >
                      <div className="min-w-[200px] p-1 text-slate-900">
                        ...
                        <p className="mt-1 text-xs">
                          Inpatient util:{" "}
                          {Math.round(selectedStats.utilization * 100)}%
                        </p>
                        <p className="text-xs">
                          Available beds: {selectedStats.availableBeds}
                        </p>
                        <p className="text-xs">
                          ICU util: {selectedStats.icuUtilization}%
                        </p>
                        <p className="text-xs">
                          Est. wait: {selectedStats.waitMins} min
                        </p>
                        {selectedHospital.distanceMiles != null && (
                          <p className="text-xs">
                            Drive: {selectedHospital.distanceMiles} mi /{" "}
                            {selectedHospital.etaMins} min
                          </p>
                        )}
                      </div>
                    </InfoWindowF>
              ...
              <div
                ...
                onClick={
                  !editingLocation
                    ? () => {
                        setLocationDraft(resolvedAddress);
              ...
              >
                ...
                {editingLocation ? (
                  <Autocomplete
                    onLoad={(a) => {
                      locationAutocompleteRef.current = a;
                    }}
                  ...
                  >
                    <input
                      ...
                      onBlur={() =>
                        setTimeout(() => setEditingLocation(false), 200)
                      }
                  ...
                  <p
                    className={`mt-0.5 text-xs leading-snug ${resolvedAddress ? "text-slate-200" : "text-slate-500 italic"}`}
                  >
                    {resolvedAddress ||
                      (locationDetecting
                        ? "Detecting…"
              ...
              <div className="pointer-events-none absolute bottom-3 right-3 max-w-[240px] rounded-lg border border-slate-600/90 bg-slate-950/85 p-3 backdrop-blur">
                ...
                <ul className="mt-2 space-y-1.5 text-xs text-slate-300">
                  ...
                  <li>
                    <span className="mr-2 inline-block h-[2px] w-6 bg-sky-400 align-middle" />
                    Top recommendation
                  </li>
                  <li>
                    <span className="mr-2 inline-block h-[2px] w-6 border-b border-dashed border-slate-400 align-middle" />
                    Closest baseline
                  </li>
                </ul>
              </div>
            ...
            <aside className="flex min-h-0 flex-col gap-4">
              <section className="rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`font-mono text-[10px] uppercase tracking-[0.2em] ${voiceEnabled ? "text-emerald-300" : "text-slate-500"}`}
                  >
                ...
                >
                  <span
                    className={`inline-block h-2.5 w-2.5 rounded-full ${voice.isListening ? "animate-pulse bg-white" : "bg-slate-950/60"}`}
                  />
                  {voice.isListening
                ...
                {locationError && (
                  <p className="mt-1 text-xs text-red-300">{locationError}</p>
                )}
              </section>
              ...
              <section className="max-h-64 overflow-y-auto rounded-xl border border-slate-700 bg-slate-950/70 p-4">
                ...
                {!voice.isListening &&
                  ...
                  !patientSummary && (
                    <p className="mt-2 text-xs text-slate-500">
                      Transcript appears here after voice intake. Click the map
                ...
                {patientSummary && (
                  <div className="mt-2 space-y-1.5 text-xs text-slate-200">
                    ...
                    <div className="mt-1 space-y-1 rounded-lg border border-slate-700/50 bg-slate-900/40 p-2">
                      <p className="font-mono text-[10px] uppercase tracking-wide text-cyan-300">
                        Extracted
                      </p>
                      {patientSummary.name && (
                        <p>
                      ...
                      {patientSummary.condition && (
                        <p>
                          Condition:{" "}
                          <span className="font-semibold text-white">
                            {patientSummary.condition.toUpperCase()}
                          </span>
                        </p>
                      )}
                      ...
                      {patientSummary.insuranceProvider && (
                        <p>
                          Insurance:{" "}
                          <span className="font-semibold text-white">
                            {patientSummary.insuranceProvider}
                      ...
                      {(patientSummary.bloodPressure ||
                        patientSummary.heartRate ||
                        patientSummary.oxygenSaturation ||
                        patientSummary.respiratoryRate) && (
                        <p>
                          Vitals:{" "}
                          {[
                            patientSummary.bloodPressure &&
                              `BP ${patientSummary.bloodPressure}`,
                            patientSummary.heartRate &&
                              `HR ${patientSummary.heartRate}`,
                            patientSummary.oxygenSaturation &&
                              `SpO₂ ${patientSummary.oxygenSaturation}%`,
                            patientSummary.respiratoryRate &&
                              `RR ${patientSummary.respiratoryRate}`,
                          ]
                            .filter(Boolean)
                            .join(" · ")}
                        </p>
                      ...
                      {patientSummary.confidence != null && (
                        <p>
                          Confidence:{" "}
                          <span className="font-semibold text-white">
                            {Math.round(patientSummary.confidence * 100)}%
                          </span>
                        </p>
                      )}
                    </div>
              ...
              <section
                className={`flex-1 min-h-0 overflow-auto rounded-xl border border-slate-700 bg-slate-950/70 p-4 transition-opacity duration-500 ${handoffDone ? "opacity-30" : "opacity-100"}`}
              >
                <h2 className="text-lg font-semibold">
                  Top 3 Hospital Choices
                </h2>
                {route && (
                  <div className="mt-2 space-y-3 text-sm">
                    {route.closest && (
                      <p>
                        ...
                        ({route.closest.distanceMiles} mi,{" "}
                        {route.closest.etaMins} min)
                      </p>
                    )}
                    <div className="space-y-2 pt-1">
                      {(route.top3 || []).map((candidate, index) => {
                        const chainEntry = dispatch?.chain?.find(
                          (e) => e.hospitalId === candidate.id,
                        );
                        return (
                          <div
                            key={candidate.id}
                          ...
                          >
                            <p className="flex flex-wrap items-center gap-2 font-medium text-slate-100">
                              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-cyan-500/20 text-sm font-bold text-cyan-300">
                                {rankLabels[index]}
                              </span>
                              <span
                              ...
                              {chainEntry?.requestStatus === "diverted" && (
                                <span className="rounded-full border border-amber-400/40 bg-amber-400/15 px-2 py-0.5 text-xs font-semibold text-amber-300">
                                  Rerouted
                                </span>
                              )}
                              {chainEntry?.requestStatus === "accepted" &&
                                chainEntry?.autoApproved && (
                                  <span className="rounded-full border border-emerald-500/40 bg-emerald-500/15 px-2 py-0.5 text-xs font-semibold text-emerald-300">
                                    Auto-Accepted
                                  </span>
                                )}
                              {chainEntry?.requestStatus === "accepted" &&
                            ...
                            <p className="mt-1 text-xs text-slate-300">
                              {candidate.distanceMiles} mi ({candidate.etaMins}{" "}
                              min) | {candidate.availableBeds} beds avail (
                              {Math.round(candidate.utilization * 100)}% util) |{" "}
                              {candidate.waitMins} min wait
                            </p>
                            {index === 0 &&
                              route.top3[1] &&
                              buildPickedOverDiff(
                    ...
                    {!dispatch ? (
                      ...
                      <div
                        className={`mt-2 rounded-md border p-3 text-sm ${dispatchPanelClass(dispatch.status)}`}
                      >
                        {dispatch.status === "active" && (
                          <>
                            <p className="font-semibold">
                              Dispatched →{" "}
                              {dispatch.currentHospital?.hospitalName}
                            </p>
                        ...
                        {dispatch.status === "accepted" && (
                          <>
                            <p
                              className={`font-semibold transition-opacity duration-500 ${handoffDone ? "opacity-0" : "opacity-100"}`}
                            >
                              Confirmed — en route to{" "}
                              {dispatch.currentHospital?.hospitalName}
                            </p>
                            <button
                              type="button"
                        ...
                        {dispatch.status === "exhausted" && (
                          <>
                            <p className="font-semibold">
                              All hospitals diverted — contact dispatch
                            </p>
                            <button
                              type="button"
                              onClick={handlePatientHandoff}
        ...
        {activeTab === "hospital" && (
          <HospitalView
            ...
            onDelete={async (request) => {
              await apiFetch(`/api/patients/${request.patientId}`, {
                method: "DELETE",
              });
              fetchHospitalRequests();
            }}
...
function buildVoiceReply(summary, resolvedOrigin, geocoded) {
  ...
  if (geocoded) {
    ...
    parts.push(
      `I couldn't pin "${summary.address}" on the map. Confirm the address.`,
    );
  }
  ...
  try {
    await apiFetch(`/api/patients/${patientId}/route`, {
      ...
      body: JSON.stringify({
        ...
        routingReason:
          "Best ranked hospital based on ETA, wait time, capacity, and status.",
      }),
    });
  } catch {
    /* non-blocking */
...
function patientToRequestCard(patient, divertEntry = null) {
  if (divertEntry) {
    return {
      ...
      requestedAt:
        divertEntry.divertedAt ?? patient.createdAt ?? new Date().toISOString(),
      acceptedAt: null,
      deliveredAt: null,
    };
  }
  ...
  return {
    ...
    hospitalName:
      patient.assignedHospitalName ?? patient.recommendedHospitalName,
    status,
    insurance: patient.insuranceProvider,
    patientSummary: patient,
    etaMins: patient.etaMinutes,
    requestedAt:
      patient.updatedAt ?? patient.createdAt ?? new Date().toISOString(),
    ...
    <section className="min-h-0 overflow-auto">
      <div className="mb-4 rounded-md border border-amber-500/30 bg-amber-500/10 p-4">
        <p className="text-sm font-semibold text-amber-200">
          Demo Admin — Hospital Status Overrides
        </p>
        <p className="mt-1 text-xs text-amber-300/70">
          Force hospital status to test the diversion chain. Overrides block
          auto-approve so requests come in as pending.
        </p>
      </div>
      {hospitals.length === 0 && (
        <p className="text-sm text-slate-400">
          Search for a location on the EMT tab first to load hospitals.
        </p>
      )}
        ...
        {hospitals.map((h) => (
          ...
          >
            ...
            <div className="mt-1 flex items-center gap-2 text-xs">
              {h.override && (
                <span
                  className={`rounded-full px-2 py-0.5 font-semibold ${statusBadgeColor(h.override)}`}
                >
                  Override: {h.override}
                </span>
              )}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {statusOptions.map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    onOverride(h.hospitalId, h.override === s ? null : s)
                  }
    ...
    <section className="grid min-h-0 grid-rows-[auto_1fr] gap-4">
      <div className="flex flex-col gap-3 rounded-xl border border-slate-700 bg-slate-950/70 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-300">
            {hospitalName ? `${hospitalName} — ` : ""}EMT notifications routed
            to your hospital. Auto-approved when capacity is healthy.
          </p>
        </div>
      </div>
        ...
        {requests.length === 0 ? (
          <div className="flex h-64 items-center justify-center rounded-xl border border-slate-700 bg-slate-950/70">
            <p className="text-base text-slate-400">
              No incoming requests for your hospital.
            </p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {requests.map((req) => (
              <div
                key={req.requestId}
                className="relative rounded-xl border border-slate-700 bg-slate-900/60 p-4"
              >
                <button
                  type="button"
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {req.escalatedFrom && (
                    <span className="rounded-full border border-orange-400/30 bg-orange-400/10 px-2.5 py-1 text-xs font-semibold text-orange-200">
                  ...
                  {req.status === "accepted" && !req.autoApproved && (
                    <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-semibold text-emerald-200">
                      Accepted
                    </span>
                  )}
                  {req.status === "diverted" && (
                    <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2.5 py-1 text-xs font-semibold text-red-200">
                      Diverted{req.divertedTo ? ` → ${req.divertedTo}` : ""}
                    </span>
                  )}
                  {req.status === "pending" && (
                ...
                <p className="mt-3 font-semibold text-slate-100">
                  {(() => {
                    ...
                    if (p?.chiefComplaint || p?.summary)
                      parts.push(p.chiefComplaint || p.summary);
                ...
                <div className="mt-2 space-y-1 text-sm text-slate-300">
                  <p>
                    ...
                    <span className="text-slate-100">
                      {req.etaMins != null ? `${req.etaMins} min` : "--"}
                    </span>
                  </p>
                  {req.insurance && (
                    <p>
                      Insurance:{" "}
                      <span className="text-slate-100">{req.insurance}</span>
                    </p>
                  )}
                  {(req.status === "accepted" || req.status === "delivered") &&
                    req.acceptedAt && (
                      <div
                        className={`mt-2 flex items-center justify-between rounded-md border px-3 py-2 ${req.status === "delivered" ? "border-emerald-500/20 bg-emerald-500/10" : "border-amber-500/20 bg-amber-500/10"}`}
                      >
                        <span
                          className={`text-xs ${req.status === "delivered" ? "text-emerald-300" : "text-amber-300"}`}
                        >
                          {req.status === "delivered"
                            ? "Total time"
                            : "En route"}
                ...
                {req.status === "pending" && (
                  <div className="mt-4 flex gap-2">
                    <button
                      ...
                      className="flex-1 rounded-md bg-emerald-500/20 px-3 py-2 text-xs font-semibold text-emerald-200 transition hover:bg-emerald-500/30"
                    >
                      Accept
                    </button>
                    <button
                      type="button"
                      onClick={() => onDivert(req)}
                      className="flex-1 rounded-md bg-red-500/20 px-3 py-2 text-xs font-semibold text-red-200 transition hover:bg-red-500/30"
                    >
                      Divert
                    </button>
                  </div>
        ...
        >
          ...
          >
            ...
            <p className="mt-0.5 text-base font-semibold text-slate-100">
              {reportReq.hospitalName}
            </p>
            <p className="mb-4 text-xs text-slate-400">
              {new Date(reportReq.requestedAt).toLocaleString()}
            </p>

            <div className="space-y-4 text-sm">
              {(reportReq.patientSummary?.name ||
                reportReq.patientSummary?.age ||
                reportReq.patientSummary?.sex) && (
                <div>
                  ...
                  {(reportReq.patientSummary.age ||
                    ...
                    <p className="text-slate-300">
                      {[
                        reportReq.patientSummary.age &&
                          `${reportReq.patientSummary.age}yo`,
                        reportReq.patientSummary.sex,
                      ]
                        .filter(Boolean)
                        .join(", ")}
              ...
              {(reportReq.patientSummary?.chiefComplaint ||
                reportReq.patientSummary?.condition) && (
                <div>
                  <p className="mb-1 text-[10px] uppercase tracking-wide text-slate-500">
                    Chief Complaint
                  </p>
                  <p className="text-slate-100">
                    {reportReq.patientSummary.chiefComplaint ??
                      reportReq.patientSummary.condition?.toUpperCase()}
                  </p>
                </div>
              )}
              ...
              {(reportReq.patientSummary?.bloodPressure ||
                reportReq.patientSummary?.heartRate ||
                reportReq.patientSummary?.oxygenSaturation ||
                reportReq.patientSummary?.respiratoryRate) && (
                <div>
                  <p className="mb-1 text-[10px] uppercase tracking-wide text-slate-500">
                    Vital Signs
                  </p>
                  <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-slate-100">
                    ...
                    {reportReq.patientSummary.heartRate && (
                      <p>
                        HR{" "}
                        <span className="font-medium text-white">
                          {reportReq.patientSummary.heartRate} bpm
                        </span>
                      </p>
                    )}
                    {reportReq.patientSummary.oxygenSaturation && (
                    ...
                    {reportReq.patientSummary.respiratoryRate && (
                      <p>
                        RR{" "}
                        <span className="font-medium text-white">
                          {reportReq.patientSummary.respiratoryRate}/min
                        </span>
                      </p>
                    )}
                  </div>
              ...
              <div>
                ...
                <p className="text-slate-100">
                  {reportReq.etaMins != null ? `${reportReq.etaMins} min` : "—"}
                </p>
              </div>
              {(reportReq.insurance ||
                reportReq.patientSummary?.insuranceProvider) && (
                <div>
                  <p className="mb-1 text-[10px] uppercase tracking-wide text-slate-500">
                    Insurance
                  </p>
              ...
              {!reportReq.patientSummary && (
                <p className="text-slate-400">
                  No detailed patient information available for this request.
                </p>
              )}
...
function nodeUtilization(node) {
  return node.beds?.inpatient_utilization != null
    ? node.beds.inpatient_utilization / 100
    : 0.2;
}
...
function getHospitalStats(hospital) {
  ...
  return {
    utilization:
      ...
      (b.inpatient_utilization != null ? b.inpatient_utilization / 100 : 0),
    availableBeds:
      hospital.availableBeds ??
      Math.round((b.inpatient_total ?? 0) - (b.inpatient_used ?? 0)),
    icuUtilization: b.icu_utilization ?? "--",
    icuAvailable: Math.round((b.icu_total ?? 0) - (b.icu_used ?? 0)),
    waitMins:
      hospital.waitMins ??
      Math.round(10 + (b.inpatient_utilization ?? 0) * 0.5),
  };
}
...
const mapOptions = {
  ...
  styles: [
    ...
    { elementType: "labels.text.fill", stylers: [{ color: "#9fb4d5" }] },
    {
      ...
      stylers: [{ color: "#1a2a43" }],
    },
    {
      featureType: "road.highway",
      elementType: "geometry",
      stylers: [{ color: "#1d3c69" }],
    },
    {
      featureType: "water",
      elementType: "geometry",
      stylers: [{ color: "#051326" }],
    },
